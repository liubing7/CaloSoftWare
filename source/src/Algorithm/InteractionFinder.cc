...
namespace algorithm
{
void InteractionFinder::Run(std::vector<caloobject::CaloCluster2D*> &clusters, const std::vector<float> params)
{
	...
	for(std::vector<caloobject::CaloCluster2D*>::iterator it=clusters.begin(); it!=clusters.end(); ++it){
		...
		for(std::vector<caloobject::CaloCluster2D*>::iterator jt=it+1; jt!=clusters.end(); ++jt){
